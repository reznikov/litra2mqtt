...
impl fmt::Display for DeviceHardwareEvent {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            DeviceHardwareEvent::ColorTemperatureInKelvin(value) => write!(f, "Color temperature {} K", value),
...
impl DeviceHardwareEvent {
    pub fn from_bytes(bytes: &[u8; 6]) -> Self {
        match bytes {
            [0x11, 0xFF, 0x04, 0x00, b1, _] => {
                DeviceHardwareEvent::Power(*b1 == 1)
            }
            [0x11, 0xFF, 0x04, 0x1C, _, _] => {
